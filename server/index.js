...
app.put('/updateCollection', async(req, res) => {
  try {
    ...
    const doc = await Collection.findOneAndUpdate({ collectionID: id },{isDeployed:true,deployedAddress:address},{ new: true });
    res.json(doc);
  } catch (error) {
    console.error(error);
    res.status(500).send("Server Error");
...
app.put('/updateCollectionPreSale', async(req, res) => {
  try {
    ...
    const doc = await Collection.findOneAndUpdate({ collectionID: id },{preSaleLive: preSaleLive},{ new: true });
    res.json(doc);
  } catch (error) {
    console.error(error);
    res.status(500).send("Server Error");
...
app.put('/updateCollectionPublicSale', async(req, res) => {
  try {
    ...
    const doc = await Collection.findOneAndUpdate({ collectionID: id },{publicSaleLive: publicSaleLive},{ new: true });
    console.log(doc);
    res.json(doc);
  } catch (error) {
    console.error(error);
...
app.put('/updateCollectionTotalWeiEarned', async(req, res) => {
  try {
    ...
    const doc = await Collection.findOneAndUpdate({ collectionID: id },{ $inc: { totalWeiEarned: totalWeiEarned } },{ new: true });
    res.json(doc);
  } catch (error) {
    console.error(error);
    res.status(500).send("Server Error");
...
app.put('/updateCollectionPreReveal',upload.fields([
  { name: 'preRevealImage', maxCount: 1 },
]), async(req, res) => {
  try {
    ...
    const doc = await Collection.findOneAndUpdate({ collectionID: id },
