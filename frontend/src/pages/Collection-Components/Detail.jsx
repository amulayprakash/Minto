...
export default function Detail() {
  ...
  const handleDeployContract = async(event) => {
    ...
    const dropkitpass = new ethers.Contract(DropKitPassAddress, DropKitPass.abi, signer);
    console.log(userAddress)
    const result = await dropkitpass.getActivatedTokenByOwner(userAddress)
    console.log(result);
    // if(result===0)
    const factory = new ethers.ContractFactory(DropCollection.abi, DropCollection.bytecode, signer);

    const contract = await factory.deploy(collection.name,collection.symbol,collection.primary,collection.secondary,collection.rpercent);
    await contract.deployed();

    console.log('Contract deployed successfully', contract.address);
    // setAddress(contract.address)
    try {
      const response = await axios.put(`http://localhost:4000/updateCollection?collectionID=${id}&address=${contract.address}`);
  ...
  const handleStartPreSale= async(event)=>{
    ...
    const transaction = await myContract.startSale(NFTCount,quantityT,quantityW,perNFTPrice,true);
    toast('Transaction Started');
    handleModal2Close(); 
    await transaction.wait();
    try {
      const response = await axios.put(`http://localhost:4000/updateCollectionPreSale?collectionID=${id}&preSaleLive=true`);
      console.log(response.data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleStartPublicSale= async(event)=>{
    ...
    const signer = provider.getSigner();
    const myContract = new ethers.Contract(collection.deployedAddress, DropCollection.abi, signer);
    const transaction = await myContract.startSale(NFTCountPublic,quantityTPublic,quantityWPublic,perNFTPricePublic,false);
    toast('Transaction Started');
    handleModal5Close(); 
    await transaction.wait();
    try {
      const response = await axios.put(`http://localhost:4000/updateCollectionPublicSale?collectionID=${id}&publicSaleLive=true`);
      console.log(response.data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleStopPreSale= async(event)=>{
    ...
    await transaction.wait();
    try {
      const response = await axios.put(`http://localhost:4000/updateCollectionPreSale?collectionID=${id}&preSaleLive=false`);
      console.log(response.data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleStopPublicSale= async(event)=>{
    ...
    await transaction.wait();
    try {
      const response = await axios.put(`http://localhost:4000/updateCollectionPublicSale?collectionID=${id}&publicSaleLive=false`);
      console.log(response.data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleCheckBoxChange = (e) => {
    ...
    setFormData((prevFormData) => ({ ...prevFormData, textAreaEnabled: checked }));
  };
      ...
      <div >
        {isLoading ? (
            ...
            {/* <div className="detail-banner">
                <img className="detail-banner-img" src={`http://localhost:4000/uploads/${collection.banner}`} alt="Banner" />
                <div className='detail-image-div'>
                    <img className="detail-image" src={`http://localhost:4000/uploads/${collection.image}`}/>
                </div>
            </div> */}
            <div className="detail-deploy-button">
            {collection.isDeployed ? <>
              <Button className="create-dropdown"  onClick={handlePrepareDropClick} size="md" variant="light">PREPARE DROP</Button>
              <Button className="create-dropdown"  onClick={handleButtonClick} size="md" variant="light">WITHDRAW</Button>
            </>
            :
            <Button className="create-dropdown"  onClick={handleButtonClick} size="md" variant="dark">CREATE CONTRACT</Button>
            }

            </div>
            <div className="MuiBox-root">
            <h2 className="detail-collection-name">{collection.name}</h2><br></br>
            <h6 >{collection.symbol} . {collection.rpercent}% Royalty . {collection.network}</h6>
            <h6 >{collection.description}</h6><br></br>

            <Nav fill variant="tabs" defaultActiveKey="/home" activeKey={activeKey} onSelect={handleSelect}>
              <Nav.Item>
            ...
            <br></br>
              {activeKey === '/overview' && <Overview collection={collection} />}
              {activeKey === '/members' && <Members />}
              {activeKey === '/drop' && <Drop collection={collection} />}
              {activeKey === '/waitlist' && <Waitlist collection={collection}/>}
              ...
              <Modal.Body>
                {
                  ...
                  <>
                <Button style={{ display: 'block', width: '100%', boxSizing: 'border-box' }} onClick={handleStopPublicSale} variant="dark">STOP PUBLIC SALE</Button> <br></br>
                <Button style={{ display: 'block', width: '100%', boxSizing: 'border-box' }} onClick={handleViewPublicMintPage} variant="dark">VIEW PUBLIC MINT PAGE</Button> <br></br>
                  </>:
                  <>
                <Button style={{ display: 'block', width: '100%', boxSizing: 'border-box' }} onClick={handleWailistMoadalClick} variant="dark" disabled>WAITLIST</Button> <br></br>
                {
                  !collection.preSaleLive ?
                  <>
                  <Button style={{ display: 'block', width: '100%', boxSizing: 'border-box' }} onClick={handlePreSaleClick} variant="dark" disabled>START PRESALE</Button> <br></br>
                  </>
                  :
                  <>
                  <Button style={{ display: 'block', width: '100%', boxSizing: 'border-box' }} onClick={handleStopPreSale} variant="dark" disabled>STOP PRESALE</Button> <br></br>
                  </>
                }
                <Button style={{ display: 'block', width: '100%', boxSizing: 'border-box' }} onClick={handlePublicSaleClick} variant="dark">PUBLIC SALE</Button>
                  </>
                }
              </Modal.Body>
              <Modal.Footer>
              ...
              <Modal.Body>
                <Form className='form-class'>
                  <Form.Group className="mb-3" controlId="formBasic">
                    ...
                    onChange={(event) => setNFTCountPublic(event.target.value)}
                    />
                  </Form.Group>
                  <Form.Group className="mb-3" controlId="formBasic">
                    <Form.Label>PRICE PER NFT</Form.Label>
                    ...
                    onChange={(event) => setPerNFTPricePublic(event.target.value)}
                    />
                  </Form.Group>
                  <Form.Group className="mb-3" controlId="formBasic">
                    <Form.Label>MAX QUANTITY PER WALLET ADDRESS</Form.Label>
                    ...
                    onChange={(event) => setQuantityWPublic(event.target.value)}
                    />
                  </Form.Group> 
                  <Form.Group className="mb-3" controlId="formBasic">
                    <Form.Label>MAX QUANTITY PER MINT TRANSACTION</Form.Label>
                    ...
                    onChange={(event) => setQuantityTPublic(event.target.value)}
                    />
                  </Form.Group> 
                </Form>
              </Modal.Body>
              <Modal.Footer>
                <Button style={{ display: 'block', width: '100%', boxSizing: 'border-box' }} variant="outline-dark" onClick={handleStartPublicSale}>START PUBLIC SALE</Button>
                <Button style={{ display: 'block', width: '100%', boxSizing: 'border-box' }} variant="dark" onClick={handleModal5Close}>CANCEL</Button>
              </Modal.Footer>
            </Modal>
            ...
            >
              ...
              <Modal.Body>
                <h5>
                  A Waitlist help you build and collect interest in your NFT project prior to your sale.
                  Please configure your waitlist requirements.
                </h5>
                <br></br>
                <Form className='form-class'>
                ...
                {formData.textAreaEnabled && (
                  <Form.Group controlId="exampleForm.ControlTextarea1">
                    <Form.Control as="textarea" rows={3} value={formData.textAreaValue} onChange={handleTextAreaChange} />
                  </Form.Group>
                )}
                </Form>
              </Modal.Body>
