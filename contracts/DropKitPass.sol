    ...
    AccessControl 
{
    ...
    mapping(uint256 => EnumerableSet.AddressSet)private _membersByToken;
    mapping(address => TokenEntry) private _tokenByMembers;

    // upgrade paths - stageId => feeRate => newStageId => newFeeRate
    mapping(uint256 => mapping(uint256 => mapping(uint256 => mapping(uint256 => PricingEntry))))
        private _upgradePaths;
    ...
    function upgradePass(
        ...
        uint256 stageId = getStage(tokenId);
        uint96 feeRate = getFeeRate(tokenId); 
        PricingEntry memory pricing = _upgradePaths[stageId][feeRate][
            newStageId
        ][newFeeRate];

        require(ownerOf(tokenId) == _msgSender(), "Not a owner");
        require(pricing.isValue, "Upgrade not eligible"); 
        require(pricing.price <= msg.value, "Invalid Price"); 

        _feeRatesByToken[tokenId] = _feeValue(newFeeRate, true);
        _stagesByToken[tokenId] = newStageId;

        emit PassUpgraded(tokenId, newStageId, newFeeRate);
    }
    ...
    function setUpgradePath(
        ...
        _upgradePaths[stageId][feeRate][newStageId][newFeeRate] = PricingEntry(
            price,
            active
        );
    ...
    function getUpgradePrice(
        ...
        return _upgradePaths[stageId][feeRate][newStageId][newFeeRate].price;
    }
    ...
    function _feeValue(uint96 feeRate, bool isValue) internal pure returns (FeeEntry memory){
